...
/*
	if (gv_options.searchbox_options && (gv_options.searchbox_options.searchbox || gv_options.searchbox_options.enabled)) {
		var pos = (gv_options.searchbox_options.position && gv_options.searchbox_options.position.length >= 3) ? gv_options.searchbox_options.position : ['G_ANCHOR_TOP_LEFT',70,6];
		var d = (gv_options.searchbox_options.draggable === false) ? false : true;
		var c = (gv_options.searchbox_options.collapsible === false) ? false : true;
		GV_Build_And_Place_Draggable_Box({base_id:searchbox_id,class_name:'gv_searchbox',position:pos,draggable:d,collapsible:c});
		// allow the search "form" to be submitted via the return key:
		if ($('gv_searchbox_input') && $('gv_searchbox_button')) { $('gv_searchbox_input').onkeypress = function(e) { if (!e) { e = window.event; } if (e.keyCode == 13) { eval($('gv_searchbox_button').getAttributeNode('onclick').nodeValue); return false; } } }
	} else if ($(searchbox_id)) {
		$(searchbox_id).parentNode.removeChild($(searchbox_id));
	}
*/
	...
	if (gv_options.center_coordinates !== false || gv_options.measurement_tools !== false) {
		...
		if (!$('gv_center_container')) {
			...
			if (gv_options.measurement_tools !== false) {
				center_html += '<td><div id="gv_measurement_icon" style="display:block; width:23px; height:15px; margin-left:3px; cursor:pointer;"><a href="javascript:void(0);" onclick="GV_Place_Measurement_Tools();" title="Click here for measurement tools" style="cursor:pointer;"><img src="'+gvg.icon_directory+'ruler.gif" width="23" height="13" border="0" vspace="1" class="gmnoprint" /></a></div></td>';
			}
...
//GV_Debug ("in GV_Finish_Map, about to possibly do 'return' if there are still files to process (gvg.dynamic_file_index = "+gvg.dynamic_file_index+")");
	if (gvg.dynamic_file_index < (gvg.dynamic_file_count-1)) {
...
function GV_Marker(arg1,arg2) {
	...
	*/
	...
	if (mi.folder && gvg.marker_list_folder_state && gvg.marker_list_folder_state[mi.folder]) {
		if (gvg.marker_list_folder_state[mi.folder].hidden) { marker.gv_hidden_by_folder = true; }
	}
		...
		if (!gvg.folders_have_been_initialized) { // brand new map; use gv_options to set state of individual folders
			...
			if (mlo && mlo.collapsed_folders && mlo.collapsed_folders.length) {
				for (var i=0; i<mlo.collapsed_folders.length; i++) {
					if (!gvg.marker_list_folder_state[mlo.collapsed_folders[i]]) { gvg.marker_list_folder_state[mlo.collapsed_folders[i]] = {}; }
					gvg.marker_list_folder_state[mlo.collapsed_folders[i]].collapsed = true; // set it even if the folder doesn't exist yet
					GV_Folder_Collapse_Toggle(mlo.collapsed_folders[i],true);
				}
			}
			if (mlo && mlo.hidden_folders && mlo.hidden_folders.length) {
				for (var i=0; i<mlo.hidden_folders.length; i++) {
					if (!gvg.marker_list_folder_state[mlo.hidden_folders[i]]) { gvg.marker_list_folder_state[mlo.hidden_folders[i]] = {}; }
					gvg.marker_list_folder_state[mlo.hidden_folders[i]].hidden = true; // set it even if the folder doesn't exist yet
					GV_Folder_Visibility_Toggle(mlo.hidden_folders[i],true);
				}
			}
		}
...
function GV_Update_Marker_List_With_Marker(m) {
	if (!m) { return false; }
	if (m.gvi.folder) {
		if (!m.gv_hidden() || m.gv_hidden_by_folder) {
...
function GV_Draw_Track(t) {
	...
	for (var s=0; s<trk[t].segments.length; s++) {
		...
		if (trk_outline_width > 0 && trk[t].segments[s].outline !== false) {
			// this is optimized so that conterminous outline segments are merged; but still, Google screws up the rendering beyond about 388 points.
			var start;
...
function GV_Add_Track_to_Tracklist(opts) {
	...
	var zoom_link = ''; if (gv_options.tracklist_options && gv_options.tracklist_options.zoom_links !== false) {
		...
		if (opts.bounds && opts.bounds.getCenter) {
			...
			zoom_link = '<img src="'+gvg.icon_directory+'tracklist_goto.gif" width="9" height="9" border="0" alt="" title="zoom to this track" onclick="gmap.setCenter(new google.maps.LatLng('+center_lat+','+center_lon+')); gmap.setZoom('+zoom+');" style="padding-left:3px; cursor:crosshair;">';
		}
	}
...
//GV_Debug ("* "+GV_Debug_Function_Name(arguments.callee)+" (called from "+GV_Debug_Function_Name(arguments.callee.caller)+")");
	...
	// gvg.folders_have_been_initialized = false; // sort of a hack, but forces the marker list to rebuild itself
...
function GV_Autozoom() { // automatically adjust the map's zoom level to cover the elements passed to this function
	...
	for (var i=1; i<arguments.length; i++) { // start at i=1 because arguments[0] was the options
		if (arguments[i]) {
			var a = (typeof(arguments[i]) == 'string') ? eval(arguments[i]) : arguments[i]; // so that 'wpts' is a valid argument
...
function GV_Control(controlDiv,anchor,padding,i) {
	...
	controlDiv.style.marginLeft = parseFloat(padding.left)+'px';
	controlDiv.style.marginRight = parseFloat(padding.right)+'px';
	controlDiv.style.marginTop = parseFloat(padding.top)+'px';
	controlDiv.style.marginBottom = parseFloat(padding.bottom)+'px';
...
function GV_Define_Styles() {
	...
	document.writeln('			#gmap_div .gv_marker_info_window { font-family:Verdana; font-size:10px; min-width:150px; min-height:50px; }');
	document.writeln('			#gmap_div .gv_marker_info_window a { font:inherit; }');
	document.writeln('			#gmap_div .gv_marker_info_window_name { font:inherit; font-size:11px; font-weight:bold; padding-bottom:4px; }');
	...
	document.writeln('			#gmap_div .gv_driving_directions { background-color:#eeeeee; padding:4px; margin-top:12px; }');
