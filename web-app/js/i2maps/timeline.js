...
(function($) {
    function Timeline(target_, options) {
        ...
        var time_target = $('<p id="time" style="text-align:center; margin:0px; padding:0px;">TIME</p>');
        plot_target.height(target_.height());
        plot_target.width(target_.width());
        ...
        function draw()
        {
            ...
            for(var d in data)
            {
                data_series.push({ color: colors[d], data: data[d] });
            }
        ...
        function setData(series, data_array)
        {
            ...
            {
                var t = dateStringToTimestamp(data_array[d].time);// + 3600000; //Add an hour - not sure why yet
        ...
        {
            return Math.round(new Date(dateString.replace(/-/g, "/")).getTime()) + 3600000;
        }
    ...
    $.timeline = function(target, options) {
        var timeline = new Timeline(target, options);
        return timeline;
    }
})(jQuery);
